...
- backend (em `backend/`): `uv run pytest`, `uv run ruff check .`, `uv run ruff format --check .`, `uv run mypy app tests/unit/application`, `uv run lint-imports`
- integracao (em `backend/`, PostgreSQL 16 real): `uv run pytest -m integration tests/integration` (testcontainers com Docker; ou `TEST_DATABASE_URL=postgresql+asyncpg://.../postgres` apontando para um servidor local)
- migracoes (em `backend/`): `uv run alembic upgrade head` / `uv run alembic check` (URL do `DATABASE_URL`)
