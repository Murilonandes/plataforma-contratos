...
| D3 ✅ | Retry antes do POST | `SAP_MAX_TENTATIVAS` (default 5) com backoff exponencial em `run_after` (30 s × 2^n, teto de 30 min) e **jitter de ±20%**. Ao atingir o teto: `FALHA_ANTES_POST_ESGOTOU`. Regra (revisada): **"o POST pode ter saído?"**. No CSRF, **qualquer** falha (inclusive `ReadTimeout`, `ReadError` e exceção não mapeada) → `FALHA_ANTES_POST` com retry/backoff; no POST, `ConnectError`/`ConnectTimeout`/`PoolTimeout` → retry. **`401`/`403` no fetch do CSRF não repete:** vai direto para `ERRO_TECNICO` (`SAP_4XX_TECNICO`, `detalhe` `fase=csrf`) com alerta, porque repetir login pode bloquear o usuário técnico no SAP. A linha nova está na tabela de classificação da §4 e tem teste. |
...
- **CSRF (D3, revisada):** qualquer falha, inclusive `ReadTimeout`/`ReadError`/não mapeada →
  `FALHA_ANTES_POST` (retry); `401`/`403` → `SAP_4XX_TECNICO` sem retry, com alerta.
- **POST antes dos bytes:** `ConnectError`, `ConnectTimeout`, `PoolTimeout` → `FALHA_ANTES_POST`.
- **Teste respx obrigatório:** conexão do pool morta que falha como `ReadError` depois da
  escrita → `CONEXAO_CAIDA_APOS_POST` → `INCERTO`.
