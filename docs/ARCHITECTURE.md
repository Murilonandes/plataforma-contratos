...
- **`Edm.Decimal` `Nullable=false` não calculado** (`ConditionRateValue`): exigido no VO (`required`), porque não existe payload válido sem ele (nem `null`, nem chave faltando). `Porcentagem`/`Valor` também são `Nullable=false`, mas vêm sempre de `calcular_parcelas`; o mapper barra `None` como segunda barreira.
