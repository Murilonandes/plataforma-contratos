...
- **Decimais:** enviar com `Content-Type: application/json;IEEE754Compatible=true` e decimais como **string**. Isso evita float na serialização. ⚠️ Validar no primeiro teste em DEV. Deixar como flag de config (`SAP_DECIMAL_AS_STRING`), passada ao mapper por parâmetro (`to_payload(contract, decimal_as_string=...)`; o mapper não lê settings).
- **Serialização:** `to_json(payload) -> bytes` (mapper) e o adapter envia `content=` (nunca `json=` do httpx). Todo `Edm.Decimal` sai com a **escala do campo fixada** (`Valor` `7766.50`, `ConditionRateValue` `1.500000000`), sem expoente e sem arredondar (casa a mais é erro), como string ou como literal numérico exato conforme a flag. `Int32` (`Parcela`) sai sempre como número.
- **Ordem das chaves:** a do `$metadata` (propriedades, `StatusBlock` depois de `CodTaxa`, navegações `to_FormPag, to_Item, to_Partner, to_PricingElement, to_Text`). Não é exigência do SAP; facilita diff em log e em teste. O mapper é gerado a partir das especificações do domínio, que seguem essa ordem.
...
- **`Edm.Decimal` `Nullable=false`** (`ConditionRateValue`, `Porcentagem`, `Valor`): exigido no VO (`required`), porque não existe payload válido sem ele (nem `null`, nem chave faltando). O mapper barra `None` como segunda barreira.
...
- **Parcelas obrigatórias de negócio (política por sales org):** na BRF1, `CustomerPaymentTerms` `Z999` exige ao menos uma parcela (`min_items` em `to_FormPag`), e toda parcela presente precisa de `Data` (`required`), mesmo o SAP aceitando sem. `TODO(decisão #16)`.
...
**Parcelas:**
- **O domínio não confia em quem montou o `to_FormPag`:** o `Contract.criar` valida por conta própria até 36 parcelas (`max_items`), `Parcela` = 1..N na ordem (`installment_out_of_sequence`), `Porcentagem` e `Valor` presentes e > 0, soma das `Porcentagem` = `100.0000` (`installment_percent_sum`) e datas presentes estritamente crescentes (`dates_not_increasing`).
...
| 16 | Contrato sem parcelas existe? Hoje, na BRF1, `Z999` exige ao menos uma parcela e toda parcela exige `Data`; outras condições de pagamento aceitam `to_FormPag` vazio | Comercial / SD | Política por sales org (§8) |
