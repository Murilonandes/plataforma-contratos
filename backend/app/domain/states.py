...
- ``sap_contract_number``: obrigatorio em ``SAP_201`` e ``RECONCILIAR_PARA_CRIADO``
  (ate 10 depois do strip, so digitos ASCII, nao so zeros, ``TODO(decisao #13)``),
  gravado na forma canonica do
...
def _numero_sap(bruto: object, regra: Regra, col: ErrorCollector) -> str | None:
    ...
    if valor is None:
        ...
        col.adicionar("sap_contract_number", ErrorCode.INVALID_FORMAT, formato="diferente de zero")
    else:
